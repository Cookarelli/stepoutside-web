...
function verifyProduction() {
  ...
  assert.equal(config.ios?.buildNumber, "40");
  ...
  for (const pod of ["react-native-maps", "RNFBApp", "RNFBAnalytics"]) {
    assert.ok(
      buildProperties[1]?.ios?.forceStaticLinking?.includes(pod),
      `${pod} must be statically linked when RN Firebase enables useFrameworks`
    );
  }
  ...
  console.log("Production validation passed: Step Outside project, iOS/Android identifiers, version 3.0.0, iOS build 40, and Android test code 7.");
}
